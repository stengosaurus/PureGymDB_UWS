...
BEGIN TRY
    ...
    VALUES ('Roger', 'Federer', '1981-08-08', 'roger.federer@example.com', 'gracefulChampion', 'customer', SYSDATETIME());
    ...
    VALUES (@customerID, 'yearly', DATEADD(DAY, -300, SYSDATETIME()), DATEADD(DAY, 65, SYSDATETIME()), 'active');

    -- Step 4: Add Payment Info
    INSERT INTO tblPayment (customerID, amount, paymentDate, paymentMethod, status)
    VALUES (@customerID, 600.00, DATEADD(DAY, -300, SYSDATETIME()), 'PayPal', 'completed');
    ...
    -- Step 6: Book a Class (Spin Class)
    DECLARE @classID INT = (SELECT classID FROM tblClass WHERE className = 'Spin Class');
    INSERT INTO tblClassBooking (customerID, classID, bookingDate, status)
    VALUES (@customerID, @classID, DATEADD(DAY, -2, SYSDATETIME()), 'attended');
    ...
    VALUES (@attendanceID, @customerID, DATEADD(HOUR, -1, SYSDATETIME()), SYSDATETIME());
    ...
    VALUES (@customerID, @attendanceID, 60, 60, 2, SYSDATETIME(), 1);

    -- Commit the transaction
    COMMIT TRANSACTION;

    PRINT 'Customer 15 and related data added successfully.';
END TRY
BEGIN CATCH
